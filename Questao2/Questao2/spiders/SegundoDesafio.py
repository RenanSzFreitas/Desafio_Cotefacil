...
class ServimedSpider(scrapy.Spider):
    ...
    def _init_(self, login: str, password: str, pedido: int, **kw) -> None:
        super(ServimedSpider, self)._init_(**kw)
        self.login = login
    ...
    def after_login(self, response: Response) -> Iterable[Request]:
        ...
        self.logger.info("O Login foi efetuado com sucesso!")
        ...
        self.logger.info(f"Buscando pelo pedido: {self.pedido}")
    ...
    def parse(self, response: Response) -> Iterable[ServimedPedidoItem]:
        if response.status != 200:
            self.logger.error(f"Falha ao buscar o pedido {self.pedido}")
            return
        ...
        if total_registros == 0:
            self.logger.error(f"O Pedido {self.pedido} não foi encontrado!")
            return

        pedido = lista[0]
        self.logger.info(f"Pedido {self.pedido} encontrado!")
        self.logger.info(f"Situação do Pedido: {pedido['situacao']}")
